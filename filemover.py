...
def get_category(file_path):
    ...
    # Se non trova una categoria, usa "Altri" + estensione
    if ext:
        return f"Altri_{ext[1:]}"  # Rimuove il punto dall'estensione
...
def scan_files(root_path):
    ...
    print(f"🔍 Scansione in corso da: {root.absolute()}")
    print("-" * 60)
...
def organize_files(root_path, dry_run=False):
    ...
    print(f"\n📊 Trovati {total_files} file in {len(files_by_category)} categorie:")
    ...
    if dry_run:
        ...
        print("\n⚠️  MODALITÀ TEST - Nessuna modifica verrà effettuata")
        return
    
    # Chiedi conferma
    print("\n" + "=" * 60)
    response = input("Vuoi procedere con l'organizzazione? (si/no): ").strip().lower()
    if response not in ['si', 's', 'sì', 'y', 'yes']:
    ...
    # Crea cartelle e copia file
    print("\n📁 Creazione cartelle e copia file...")
    original_dirs = set()
    files_copied = 0
    files_failed = 0
    ...
    for category, files in files_by_category.items():
        ...
        for file_path in files:
            ...
            try:
                ...
                files_copied += 1
                print(f"  ✓ Copiato: {file_path.name} → {category}/")
                logging.info(f"File copiato: {file_path} → {dest_file}")
            except Exception as e:
                files_failed += 1
                print(f"  ✗ Errore copiando {file_path.name}: {e}")
    ...
    print("\n🗑️  Eliminazione cartelle originali...")
    dirs_deleted = 0
    dirs_failed = 0
    
    for dir_path in original_dirs:
        # Non eliminare la root o le cartelle appena create
        if dir_path != root and dir_path.parent == root and dir_path.name not in files_by_category:
            try:
                shutil.rmtree(dir_path)
                dirs_deleted += 1
                print(f"  ✓ Eliminata: {dir_path.name}/")
                logging.info(f"Cartella eliminata: {dir_path}")
            except Exception as e:
                dirs_failed += 1
                print(f"  ✗ Errore eliminando {dir_path.name}: {e}")
